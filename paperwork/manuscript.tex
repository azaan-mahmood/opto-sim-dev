...
3.19\% vs paper~3.3\%).  The framework is fully reproducible (seeded RNG,
...
vicinity of the 2~km boundary: at $L = 2$~km the multi-section
ordered product of $\sim$2000 sections produces a net SO(3)
rotation of $0.1$--$0.3$~rad (stochastic residual dominates at
this distance), while the phenomenological model with
$L \ll L_{\text{char}}$ gives $\theta \approx 0.08$~rad.
The transition is therefore smooth and no discontinuity appears
in distance sweeps.
...
\begin{figure}[!ht]
  ...
  200,000 pulses per distance (seed~42).  Gobby paper data (green
  squares) from Fig.~3~\cite{gobby}.
  The 0~km Monte Carlo QBER = 3.19\% matches the paper's $\sim 3.3\%$
  baseline.  Statistical noise increases beyond 40~km due to limited
  sifted bits at $\mu = 0.1$.}
  \label{fig:gobby}
\end{figure}
...
simulation produces:
\begin{itemize}[nosep]
  \item \textbf{0~km: QBER = 3.19\%} (paper: $\sim$3.3\%) --- excellent
    agreement, confirming the SPAD dark count and afterpulsing models
    produce the correct noise baseline.
  \item \textbf{4--40~km: QBER = 1--3\%} --- consistent with the paper's
    $\sim$3.3\% plateau within Monte Carlo statistics.
  \item \textbf{40--122~km}: sifted bit counts drop below 100 per point
    at $\mu = 0.1$ with 200,000 pulses; the Monte Carlo points become
    statistically noisy but remain consistent with the analytical curve.
\end{itemize}

The key validation is the 0~km QBER: the simulation reproduces the
paper's baseline without any adjustable parameters in the SPAD model
beyond the published specifications~\cite{id230}.  The noise floor
at short distances is dominated by afterpulsing (5\% probability),
consistent with the detector model.
...
data (0~km~QBER = 3.19\% vs paper~3.3\%), demonstrating that the
