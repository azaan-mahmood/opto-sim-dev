...
\scriptref{src/channel/fiber.py:propagate(); src/lasers/cwlaser.py:sample\_field()}
...
\scriptref{src/channel/fiber.py, lines 96--129;
...
\scriptref{src/channel/fiber.py, lines 132--159;
...
\scriptref{src/channel/fiber.py, lines 161--182;
...
\scriptref{src/channel/fiber.py, lines 184--188;
...
\scriptref{src/channel/fiber.py:propagate(), lines 42--93 (docstring
...
\texttt{propagate()} at $z/L_D = \{0.0, 0.5, 1.0, 2.0\}$ where
...
DGD recorded directly from \texttt{propagate()} via \texttt{\_dgd\_sampled}
