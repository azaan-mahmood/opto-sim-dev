...
def sweep_distance(distances, num_bits=1000000, mu=0.1, seed=42):
...
if __name__ == "__main__":
    ...
    print(f"  mu=0.1, seed=42, bits=1,000,000 per point")
    print(f"  {'Distance':>8}  {'Loss':>6}  {'Sifted':>7}  {'Errors':>6}  {'QBER':>7}")
    print("-" * 55)
    results = sweep_distance(distances)
    ...
    ax1.set_ylabel('QBER (%)')
    ax1.set_title('QBER vs Fiber Length')
    ax1.grid(True, alpha=0.3)
    ax1.set_ylim(0, max(qber_pct) * 1.15)

    # Add reference lines from paper
    ax1.axhline(y=2.0, color='r', linestyle='--', alpha=0.5, label='Lab 50 km (paper: ~2%)')
    ax1.axhline(y=5.5, color='g', linestyle='--', alpha=0.5, label='Urban 30 km (paper: ~5.5%)')
